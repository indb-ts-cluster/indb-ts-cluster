...
/*
    experiment code for overlap length
 */
public class ExpOverlapLen {
    public static void main(String[] args) throws IoTDBConnectionException, StatementExecutionException {
        long startTime, endTime;
        String[] dataset = {"air", "ecg", "jinfeng", "tianyuan"};   // datasets
        int[] totalSizes = {714962,700000,1095360,1002240}; // data size
        int[] overlaplens = {500,1000,1500,2000,2500,3000}; // overlap len
        int cur_overlap_index = 0;  // currently testing overlaplen index
        int cur_data_index = 3; // currently testing dataset
        int[] seq_lengths = {166,140,192,96};   // sequence length corresponding to datasets
        String[] ks = {"'3'","'5'","'3'","'3'"};    // for inputing k
        String l = "'"+seq_lengths[cur_data_index]+"'"; // for inputing l
        String[] methods = {"kshapeudf", "kshapemudf", "kshape", "kshapem"};//methods
        ...
        int overlaplen = overlaplens[cur_overlap_index];
        ...
        for (int i = 0; i < 10; i++) {
            String warmup = "select kshapem(s0,'level'='chunk') from root." + curDataset + ".d0 where time>=1000 and time<5000";
        ...
        str.append(String.valueOf(overlaplen) + "\t");
